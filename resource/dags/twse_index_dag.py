...
import httpx
from models.twse_models import Base, TwseIndex, DailyQuote
from util import (
    TAIPEI_TZ,
    DataConverter,
    get_db_engine,
...
@task(task_id='scrape_twse_index_data')
def scrape_twse_index(**context) -> List[Dict]:
    try:
        ...
        with httpx.Client() as client:
            ...
            for table in data["tables"]:
                ...
                # 處理價格指數
                if table.get("title", "").startswith(f"{execution_date.year-1911}年{execution_date.month:02d}月{execution_date.day:02d}日 價格指數"):
                    for row in table["data"]:
                        try:
                            # 解析漲跌符號和價差
                            change_symbol = row[2]
                            change_value = DataConverter.convert_value(row[3])
                            ...
                            index_data.append({
                                ...
                                "index_name": row[0],
                                "closing_price": DataConverter.convert_value(row[1]),
                                "change_value": change_value,  # 合併後的漲跌點數
                                "change_percent": DataConverter.convert_value(row[4]),
                                "volume": 0,
                                "amount": 0
                            })
                        except (ValueError, IndexError) as e:
                            logger.warning(f"Error parsing price index row: {str(e)}")
                            continue
                            
                # 處理個股資料
                elif f"{execution_date.year-1911}年{execution_date.month:02d}月{execution_date.day:02d}日 每日收盤行情" in table.get("title", ""):
                    for row in table["data"]:
                        try:
                            # 解析漲跌符號和價差
                            change_symbol = row[9]
                            ...
                            logger.warning(f"Error parsing stock row: {str(e)}")
                            continue
                            
            if not index_data and not quote_data:
                raise ValueError("No data was parsed from the response")
                
            logger.info(f"Successfully parsed {len(index_data)} index records and {len(quote_data)} quote records for date {execution_date.date()}")
            return {"index_data": index_data, "quote_data": quote_data}
...
def save_index_data(data: Dict):
    try:
        ...
        # 將日期字串轉換回 Date 物件
        for record in data["index_data"]:
            record["date"] = date.fromisoformat(record["date"])
        
        # 批次獲取所有已存在的記錄
        existing_records = session.query(TwseIndex).filter(
            TwseIndex.date.in_([r["date"] for r in data["index_data"]]),
            TwseIndex.index_name.in_([r["index_name"] for r in data["index_data"]])
        ).all()
        
        # 建立已存在記錄的映射
        existing_map = {(r.date, r.index_name): r for r in existing_records}
        
        # 批次處理更新和插入
        for record in data["index_data"]:
            try:
                key = (record["date"], record["index_name"])
                if key in existing_map:
                    ...
                    # 插入新記錄
                    new_record = TwseIndex(**record)
                    session.add(new_record)
            except Exception as e:
                logger.error(f"Error processing index record {record}: {str(e)}")
                continue
        
        session.commit()
        logger.info(f"Successfully saved {len(data['index_data'])} index records to database")
...
def save_quote_data(data: Dict):
    try:
        ...
        session.commit()
        logger.info(f"Successfully saved {len(data['quote_data'])} quote records to database")
...
with DAG(
    ...
    check_trading_day_task, skip_execution_task = create_trading_day_branch(dag, 'scrape_twse_index_data')
    scrape_data_task = scrape_twse_index()
    save_index_data_task = save_index_data(scrape_data_task)
    save_quote_data_task = save_quote_data(scrape_data_task)

    create_tables_task >> check_trading_day_task
    check_trading_day_task >> [scrape_data_task, skip_execution_task]
    scrape_data_task >> [save_index_data_task, save_quote_data_task] 