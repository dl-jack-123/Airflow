...
@task(task_id='scrape_tpex_after_hours_trading_data')
def scrape_tpex_after_hours_trading(**context) -> List[Dict]:
    try:
        ...
        url = "https://www.tpex.org.tw/www/zh-tw/afterTrading/fixPricing"
        data = {
            "date": date_str,
            "id": "",
            "response": "json"
            ...
            for row in data["tables"][0]["data"]:
                try:
                    ...
                    after_hours_trading_data.append({
                        ...
                        "stock_code": row[0].strip(),  # 證券代號
                        "stock_name": row[1].strip(),  # 證券名稱
...
@task(task_id='save_tpex_after_hours_trading_data')
def save_tpex_after_hours_trading_data(data: Dict):
    try:
        engine = get_db_engine()
...
with DAG(
    'tpex_after_hours_trading_dag',
    default_args=default_args,
    description='爬取櫃買中心盤後定價交易資料',
    schedule='0 16 * * *',  # 每天16:00執行
    start_date=datetime(2024, 1, 1, tzinfo=TAIPEI_TZ),  # 設定開始時間為台北時區
    catchup=False,  # 啟用回補功能
    tags=['tpex', 'after_hours_trading'],
) as dag:

    create_tables_task = create_tables()
    check_trading_day_task, skip_execution_task = create_trading_day_branch(dag, 'scrape_tpex_after_hours_trading_data')
    scrape_data_task = scrape_tpex_after_hours_trading()
    save_data_task = save_tpex_after_hours_trading_data(scrape_data_task)
